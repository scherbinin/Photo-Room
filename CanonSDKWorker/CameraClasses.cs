...
{
    public delegate void GetImageHanlder(Image img);
    public delegate void GetStringHanlder(string str);
    ...
    public class SDKHandler : IDisposable
    {
        ...
        /// <summary>
        /// Делегат, куда будем отдавать снимок
        /// </summary>
        public GetImageHanlder ImageHandler { get; set; }
        ...
        public uint Error
        {
            ...
            set
            {
                ...
                {
                    if(_actionIfException!=null)
                        _actionIfException.Invoke();
                    else
        ...
        /// <summary>
        /// Поток фотографирования
        /// </summary>
        private Thread _photoThread;
        ...
        {
            if (_photoThread != null)
            {
                return _photoThread.IsAlive;
            }
        ...
        private Action _actionIfException;

        /// <summary>
        /// Takes a photo with the current camera settings
        /// </summary>
        public void TakePhoto(Action actionIfException)
        {
            _actionIfException = actionIfException;

            //start thread to not block everything
            _photoThread = new Thread(delegate()
            {
                int BusyCount = 0;
